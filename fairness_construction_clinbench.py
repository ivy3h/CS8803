...
def load_clinbench_data(data_dir):
    """
    从clinbench_hpb文件夹加载所有JSON文件，并提取question字段
    返回统一格式的数据列表，每个元素包含：
    {
        "source_file": 文件名,
        "original_index": 原始索引,
    ...
    for filename in json_files:
        ...
        for idx, item in enumerate(data):
            # 提取question字段（不同文件的question字段名称一致）
            if 'question' in item:
                all_data.append({
...
def main():
    ...
    print("Loading clinbench_hpb dataset...")
    DATA_DIR = "./clinbench_hpb"  # 根据实际路径调整
    dataset = load_clinbench_data(DATA_DIR)
    print("Dataset loaded.")

    # --- 4. 配置参数 ---
    NUM_SAMPLES = len(dataset)  # 设置要处理的原始样本数量
    BATCH_SIZE = 512            # 每个推理批次的大小
    OUTPUT_FILE = "./scratch/clinbench_hpb_fairness.jsonl"
