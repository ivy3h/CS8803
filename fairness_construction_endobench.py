...
def main():
    ...
    print("Loading dataset 'Saint-lsy/EndoBench'...")
    dataset = load_dataset("Saint-lsy/EndoBench", split="test")
    print(f"Dataset loaded: {len(dataset)} samples")

    # --- 4. 配置参数 ---
    NUM_SAMPLES = len(dataset)  # 设置要处理的原始样本数量
    BATCH_SIZE = 512            # 每个推理批次的大小
    OUTPUT_FILE = "./scratch/EndoBench_fairness.jsonl"
    ...
    with open(OUTPUT_FILE, "a", encoding="utf-8") as f:
        ...
        for i in tqdm(range(0, len(prompts), BATCH_SIZE), total=num_batches, desc="Processing Batches"):
            ...
            for j, output in enumerate(batch_outputs):
                ...
                # 创建新的数据条目，包含所有分组信息
                new_item = {
                    "original_index": original_item['index'],
                    **original_item,
